...
$('#ToolbarView').on('click', '.toolbar-btn', function(e) {
	...
	if (toolName === "select-btn") {
		...
		$("#PadView").css({'cursor': 'url(assets/resources/cursor-images/eraser.png) 0 22, auto'})

	} else if (toolName === "measure-btn") {
		easel.currentTool = tacit.tools.measure
		$("#PadView").css({'cursor': 'url(assets/resources/cursor-images/ruler.png), auto'})


	} else if (toolName === "test-btn") {
		easel.currentTool = tacit.tools.test
		$("#PadView").css({'cursor': 'url(assets/resources/cursor-images/weight.png), auto'})
